...
async def run_campaign(campaign_id: int, delay_seconds: int = 60):
    ...
    # Get all leads assigned to this campaign that haven't been called yet

    leads = db.get_leads(campaign_id=campaign_id, status='new', limit=500)

    # Also include 'unassigned' leads that may have been bulk-assigned

    called_leads  = db.get_leads(campaign_id=campaign_id, status='called',   limit=500)

    interest_leads = db.get_leads(campaign_id=campaign_id, status='interested', limit=500)


    # Only call 'new' leads — skip already called/interested/demo_booked

    to_call = leads  # only new leads


    if not to_call:

        logger.info(f"Campaign {campaign_id}: no new leads to call")

        db.add_log(f"⚠️ Campaign {campaign_id}: no new leads found to call")
